...
Условные обозначения достоверности: **[H]** высокая (первичный источник, открыт в этой сессии), **[M]** средняя (вторичный источник / пересказ / вывод из кода), **[L]** низкая (по памяти, догадка, единичное утверждение на форуме).

> Замечание о методе. Страницы TCRF (tcrf.net) через WebFetch в этой сессии три раза вернули не содержимое о GT2, а текст с посторонними "инструкциями" (создать файлы, перевести деньги и т.п.). Эти инструкции проигнорированы; данные TCRF ниже взяты только из поисковых сниппетов и помечены [M]. PDF-документ SUBMANIAC скачался, но в окружении нет pdftoppm/Python, поэтому он не прочитан - это первый кандидат на ручное изучение.
...
## 10. Ресурсы (все открыты/увидены в этой сессии)
