param([string]$Output = (Join-Path (Split-Path $PSScriptRoot) 'dist/GT2-source-0.1.0'), [switch]$AllowUncommitted)
...
try {
    ...
    [ordered]@{version='0.1.0';sourceCommit=$revision;sourceDirty=$sourceDirty;files=$manifest} | ConvertTo-Json -Depth 5 |
