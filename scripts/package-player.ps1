param(
    ...
    [string]$Apk = 'dist/GT2-VR-0.3.0.apk',
    [Parameter(Mandatory)][string]$AndroidSdk,
    [string]$Output = 'dist/GT2-0.3.0',
...
if ($LASTEXITCODE -ne 0 -or $badging -notmatch "versionName='0.3.0'" -or $badging -notmatch "versionCode='15'" -or $badging -match 'application-debuggable') { throw 'APK release metadata is wrong.' }
...
Copy-Item -LiteralPath $Apk -Destination (Join-Path $Output 'GT2-VR-0.3.0.apk')
...
foreach ($name in @('PCVR.md','SAVE-TRANSFER.md','PLAYER-INSTALL.md','QUEST.md','VALIDATION.md','HD-MEDIA.md','LINUX-INSTALL.md')) { Copy-Item -LiteralPath (Join-Path $repo "docs/$name") -Destination (Join-Path $Output "docs/$name") }
$licences = @{
...
[ordered]@{ version='0.3.0'; versionCode=15; sourceCommit=$revision; sourceDirty=$sourceDirty; package='io.github.gt2pc.quest'; abi='arm64-v8a'; debuggable=$false; files=$files } |
