...
if ($manifest.version -ne '0.2.0') { throw 'Unexpected release version.' }
$seen = @{}
...
foreach ($required in @('GT2-VR-0.2.0.apk','tools/gt2game.exe','tools/gt2install.exe','tools/gt2checks.exe','scripts/install-player.ps1','scripts/install.ps1','scripts/install-quest.ps1')) {
...
if ($Target -eq 'Quest') {
    ...
    & (Join-Path $PSScriptRoot 'install-quest.ps1') -Adb $adbPath -Serial $Serial -Apk (Join-Path $root 'GT2-VR-0.2.0.apk') -Runtime $InstallDir -Mode $modes[0] -BothDiscs:($modes.Count -eq 2)
